...
export default function CRM({ entries, currentUser, isBoard, isSaleManager, isNVKD, userBranch, SALE_BY_BRANCH, BRANCHES, SPECIALTIES }) {
  ...
  // Scope by role
  const scopedCustomers = isBoard || !isNVKD
    ? customers
    : customers.filter(k => k.nvkd === currentUser);
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:10}}>
        <div>
          ...
          <p style={{color:"#6b7280",fontSize:13,marginLeft:14}}>
            {scopedCustomers.length} KH · {reminders.length > 0 ? <span style={{color:RED,fontWeight:700}}>⚠ {reminders.length} cần follow-up</span> : "Không có reminder"}
          </p>
        </div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        {(isBoard||isSaleManager) && (
          <select value={filterNVKD} onChange={e=>setFilterNVKD(e.target.value)}
            ...
            {allNVKDs.map(n=><option key={n}>{n}</option>)}
          </select>
        )}
